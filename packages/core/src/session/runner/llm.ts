...
const MAX_STEPS = 25
const MAX_VERIFICATION_STEPS = 15
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const phaseVerification = yield* PhaseVerification.Service
    ...
    const resolveSessionModel = Effect.fn("SessionRunner.resolveSessionModel")(function* (sessionID: SessionSchema.ID) {
      const session = yield* getSession(sessionID)
      return yield* models.resolve(session)
    })

    const runLoop = Effect.fn("SessionRunner.runLoop")(function* (
      sid: SessionSchema.ID,
      classified: boolean,
      initialPromotion: SessionInput.Delivery | undefined,
      initialActivity: boolean,
    ) {
      let promotion: SessionInput.Delivery | undefined = initialPromotion
      let openActivity = initialActivity
      while (openActivity) {
        let needsContinuation = true
        let modelSteps = 0
        let verificationSteps = 0
        let isVerificationTurn = false
        while (modelSteps < MAX_STEPS) {
          // ── Snapshot phase state before turn ──
          const phaseSnapshot = yield* phaseVerification.snapshot(sid)

          needsContinuation = yield* runTurn(sid, promotion)
          promotion = "steer"

          if (!isVerificationTurn) modelSteps++
          isVerificationTurn = false

          // ── Complexity gate: skip verification for simple tasks ──
          const simple = classified && (yield* phaseVerification.isSimple(sid))

          if (!needsContinuation) {
            // ── Loop Closure: detect phase completion + fallback + PGO loop ──
            const {
              singlePhase,
              completedPhaseName,
              allPhasesComplete,
              ...
              pgoContinueRound,
              gitSettled,
            } = yield* phaseVerification.detectPhaseCompletion(sid, phaseSnapshot)

            const canVerify = verificationSteps < MAX_VERIFICATION_STEPS

            if (!simple) {
              if ((allPhasesComplete || gitSettled) && canVerify) {
                // Global PGO: harness runs typecheck/lint + sub-agent review
                const model = yield* resolveSessionModel(sid)
                yield* phaseVerification.injectGlobalVerification(sid, location.directory, model)
                needsContinuation = true
                verificationSteps++
                isVerificationTurn = true
              } else if (pgoContinueRound && canVerify) {
                // PGO loop: model tightened, harness re-ran checks, results changed → continue
                needsContinuation = true
                verificationSteps++
                isVerificationTurn = true
              } else if ((singlePhase || hasGitChanges) && canVerify) {
                // Local loop closure: phase completed or git changes detected
                yield* phaseVerification.injectLocalVerification(sid, completedPhaseName)
                needsContinuation = true
                verificationSteps++
                isVerificationTurn = true
              }
            } else {
              // Simple task: final verification only if git changes detected
              if (hasGitChanges && verificationSteps < 1) {
                const model = yield* resolveSessionModel(sid)
                yield* phaseVerification.injectGlobalVerification(sid, location.directory, model)
                needsContinuation = true
                verificationSteps++
                isVerificationTurn = true
              } else if (pgoContinueRound && verificationSteps < MAX_VERIFICATION_STEPS) {
                yield* phaseVerification.upgradeToComplex(sid)
                needsContinuation = true
                verificationSteps++
                isVerificationTurn = true
              }
          ...
          if (!needsContinuation) {
            const settled = yield* plugin.trigger(
              "runner.turn.settled",
              { sessionID: sid, text: "", cwd: location.directory },
              { synthetic: [] },
            )
            for (const msg of settled.synthetic) {
          ...
          if (!needsContinuation) needsContinuation = yield* SessionInput.hasPending(db, sid, "steer")
          if (!needsContinuation) break
        }
        if (needsContinuation)
          return yield* new StepLimitExceededError({ sessionID: sid, limit: MAX_STEPS })
        openActivity = yield* SessionInput.hasPending(db, sid, "queue")
    ...
    const run = Effect.fn("SessionRunner.run")(function* (input: {
      ...
      const hasSteer = yield* SessionInput.hasPending(db, sid, "steer")
      const hasQueue = hasSteer ? false : yield* SessionInput.hasPending(db, sid, "queue")
      if (input.force !== true && !hasSteer && !hasQueue) return
      yield* failInterruptedTools(sid)

      // ── Complexity gate: classify once at session start ──
      let classified = false
      if (!hasQueue) {
        const context = yield* store.context(sid)
        const firstUser = context.find((m) => m.type === "user")
        if (firstUser && firstUser.type === "user" && firstUser.text) {
          const model = yield* resolveSessionModel(sid)
          yield* phaseVerification.classify(sid, firstUser.text, model)
          classified = true
        }
      }

      // ── Error baseline: collect BEFORE model makes changes ──
      yield* phaseVerification.establishBaseline(sid, location.directory)

      const promotion: SessionInput.Delivery | undefined = hasSteer ? "steer" : hasQueue ? "queue" : undefined
      const openActivity = input.force === true || hasSteer || hasQueue
      yield* runLoop(sid, classified, promotion, openActivity).pipe(
        Effect.ensuring(phaseVerification.dispose(sid)),
      )
    })
