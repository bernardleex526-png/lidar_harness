...
import { PhaseVerification } from "@opencode-ai/core/session/runner/phase-verification"
...
const phaseVerification = Layer.succeed(
  PhaseVerification.Service,
  PhaseVerification.Service.of({
    snapshot: () => Effect.succeed([]),
    classify: () => Effect.succeed("complex"),
    isSimple: () => Effect.succeed(false),
    upgradeToComplex: () => Effect.void,
    establishBaseline: () => Effect.void,
    ...
    injectLocalVerification: () => Effect.void,
    injectGlobalVerification: () => Effect.void,
    dispose: () => Effect.void,
  }),
)
const pluginV2 = Layer.succeed(
  PluginV2.Service,
  PluginV2.Service.of({
    add: () => Effect.void,
    remove: () => Effect.void,
    triggerFor: (_id, _name, input, output) => Effect.succeed({ ...input, ...output } as any),
    trigger: (_name, input, output) => Effect.succeed({ ...input, ...output } as any),
  }),
)
...
const runner = SessionRunnerLLM.layer.pipe(
  ...
  Layer.provide(phaseVerification),
  Layer.provide(pluginV2),
